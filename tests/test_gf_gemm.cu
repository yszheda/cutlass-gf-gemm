 ...
 *
 * This test suite validates the correctness and performance of
 * GF(2^8) matrix multiplication across various matrix sizes.
 */
...
/**
 * @brief Test basic matrix multiplication
 */
int test_basic_gemm() {
    printf("\n=== Test: Basic GEMM ===\n");
...
/**
 * @brief Test with identity matrix
 */
int test_identity() {
...
int test_zero_matrix() {
    ...
    // A is zero, so C should be zero regardless of B
    srand(456);
    for (size_t i = 0; i < size_b; ++i) {
        h_B[i] = rand() % 256;
    }
...
/**
 * @brief Test various matrix sizes
 */
